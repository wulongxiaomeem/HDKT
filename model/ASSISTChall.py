...
Q_matrix_c = np.loadtxt('matrix/AssistChall/new/Q_matrix_idx.txt')
Q_matrix_cd = np.loadtxt('matrix/AssistChall/new/Q_matrix_kcdifficulty.txt')
EP_matrix = np.loadtxt('matrix/AssistChall/new/pp_matrix.txt')
ED_matrix = np.loadtxt('matrix/AssistChall/new/pd_matrix.txt')
...
loading edges
'''
with open('edges/AssistChall/new/exercise-concept_edges.pkl', 'rb') as file:
    ec_edges = pickle.load(file)
with open('edges/AssistChall/new/exercise-difficulty_edges.pkl', 'rb') as file:
    ed_edges = pickle.load(file)
with open('edges/AssistChall/new/exercise-popularity_edges.pkl', 'rb') as file:
...
hg.add_hyperedges(ed_edges, group_name = "ed")
hg.add_hyperedges(ep_edges, group_name = "ep")
...
class HyKTNet(nn.Module):
    def __init__(self, num_dict, embedding_dict, hg, Q_matrix_c, Q_matrix_cd, dropout = 0.2):
        ...
        self.HgnnpConv1 = dhg.nn.HGNNPConv(self.d_e, 2 * self.d_e, bias = True, use_bn = True, drop_rate = 0.5, is_last = False)
        self.HgnnpConv2 = dhg.nn.HGNNPConv(2 * self.d_e, self.d_e, bias = True, use_bn = True, drop_rate = 0.5, is_last = True)
        ...
        self.linear_predicting_s = nn.Linear(self.d_attempt + self.d_attempt, 1)
    ...
    def forward(self, e_data, ed_data, ep_data, a_data, as_data, ha_data, ca_data, it_data,deliver_h=None, deliver_h_list=[], train_epoch = False):
        ...
        min_learing = torch.tensor(0.1).to(device)
        ...
        for t in range(0, seq_len):
            ...
            s_part
            '''
            s_pred = self.sig(self.linear_predicting_s(torch.cat((ha, ca),1)))
...
train_data = dat_total_train.load_data('dataset/AssistChall/500/new/total_train_filtered.txt')
test_data = dat_test.load_data('dataset/AssistChall/500/new/test_filtered.txt')
